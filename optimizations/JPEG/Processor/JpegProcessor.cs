...
public class JpegProcessor : IJpegProcessor
{
	...
	private static CompressedImage Compress(Matrix matrix, int quality = 50)
	{
      ...
      Parallel.For(0, matrix.Height / DCTSize, i =>
		...
		var compressedBytes = HuffmanCodec.Encode(allQuantizedBytes.OrderBy(p => p.Key).SelectMany(p => p.Value), out var decodeTable, out var bitsCount);
   ...
   private static void ShiftMatrixValues(double[,] subMatrix, int shiftValue)
		...
		int yOffset, int xOffset)
	{
		...
		for (var y = 0; y < height; y++)
		for (var x = 0; x < width; x++)
         pixels[yOffset + y, xOffset + x] = new Pixel(a[y, x], b[y, x], c[y, x], format);
	}
